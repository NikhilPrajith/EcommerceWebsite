...
export default function CollapseCard2({product}) {
    ...
    return(
        <>
        <ShoppingCart open ={checkoutOpen} setOpen={setChecoutOpen} productDetail={product}></ShoppingCart>
        ...
        <Collapse in={open} timeout="auto" unmountOnExit>
            <div style={{display:'flex', justifyContent:'flex-end'}}>
                ...
                <div style={{width:'20%',display:'flex',flexDirection:'column',justifyContent:'center',alignContent:'end'}}>
                    {("bidAccepted" in product.data && (data) && (`${data.userId}` in product.data['bidAccepted']) && (product.data['bidAccepted'][`${data.userId}`] == "Need Payment")) && <Button onClick={()=>{setChecoutOpen(true)}} style={{background:'black'}} variant="contained">Complete Payment</Button> } 
                    <Button style={{borderColor:'black', color:'black'}} onClick={()=>{router.push(`/productDetails?id=${product.prodId}`)}}  variant="outlined">Update Bid</Button>
                </div>
            </div>
        </Collapse>
        </> 
